 ...
 * @version    $Id: Consumer.php 20052 2010-01-03 15:12:37Z padraic $
 */
...
class Zend_Oauth_Consumer extends Zend_Oauth
{
        ...
        $httpMethod = null, Zend_Oauth_Http_AccessToken $request = null)
    {
        ...
        // OAuth 1.0a Verifier
        if (!is_null($authorisedToken->getParam('oauth_verifier'))) {
            $request->setParameters(array(
                'oauth_verifier' => $authorisedToken->getParam('oauth_verifier')
            ));
        }
        if (!is_null($httpMethod)) {
            $request->setMethod($httpMethod);
