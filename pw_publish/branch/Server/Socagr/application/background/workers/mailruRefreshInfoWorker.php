...
$worker->addFunction('mailruRefreshInfo', 'mailruRefreshInfo');
while (1)
{
    echo "Wait...\n";
    $ret= $worker->work();
...
function mailruRefreshInfo($job)
{
    echo "Work...\n";
    try
    {
        ...
        if(isset($data['_id']))
        {
            ...
            $user = $usersModel->getByAuid($data['_id'],array('mru.snuid','mru.sntoken','mru.snrefresh_token','mru.dt_request_sntoken','mru.dt_request_snrefresh_token','mru.gender'));
            if($user)
            {
                $mailruUsers = new MailRuUsers();
                ...
                $newTokens = $data['sntoken'] == $user['mru']['sntoken'];

                $check = $mailruUsers->validateToken($data['_id'], $data['sntoken']);
                if (isset($check['error']) && $check['error'] == 4)
                {
                    $refreshed = $mailruUsers->refreshToken($data['snrefresh_token']);
                    if (isset($refreshed['error']))
                    {
                        $message = 'MailRuRefreshInfo : ['.$refreshed['error'].'] '.$refreshed['message'].'; snuid='.$data['_id'];
                        $type = Sa_Log::ERR;
                    }
                    else
                    {
                ...
                if (!$message)
                {
                    $userFullInfo = $mailruUsers->requestFullUserData($user['mru']['snuid'], $data['sntoken'], $data['snrefresh_token']);
                    if(!isset($userFullInfo['error']))
                    {
                        ...
                        $deletedSnuids = array();
                        $deletedAuids = array();
                        if (!empty($user['mru']['gender']) && empty($userFullInfo['mru']['gender']))
                        {
                            // Keep gender since it could be empty at MailRu.
                            $userFullInfo['mru']['gender'] = $user['mru']['gender'];
                        }
                            ...
                            if($friendsModel->removeSnFriends($snid,$data['_id']))
                            {
                                if(isset($friends['friends_data']) && count($friends['friends_data']) > 0)
                                {
                                    if($friendsModel->addSnFriends($snid,$data['_id'],$friends['friends_data']))
                                    {
                                        ...
                                        # Mark me as used=1 for all my mutual friends from MailRu
                                        ...
                                        if(count($deletedSnuids) > 0)
                                        {
                                            ...
                                            {
                                                $deletedRecords[] = $snid.'_'.$deletedAuid.'_'.$user['mru']['snuid'];
                                            }
                        ...
                        {
                            $message = 'MailRuRefreshInfo : Info refresh for user with auid = '.$data['_id'].' complete.';
                        }
                    }
                    else
                    {
                        if($config['log_level'] == 'info' || $config['log_level'] == 'error')
                        {
                            ...
                            $message = 'MailRuRefreshInfo : MailRu error: '.$userFullInfo['message'];
                            $type = Sa_Log::ERR;
                        }
                    }
                }
            ...
            else
            {
                if($config['log_level'] == 'info' || $config['log_level'] == 'error')
                {
                    $message = 'MailRuRefreshInfo : MailRu account for user with auid = '.$data['_id'].' not found.';
                    $type = Sa_Log::ERR;
                }
            }
            if($message)
    ...
    {
        $message = "MailRuRefreshInfo : Error message:".$e->getMessage()."\nTrace:".$e->getTraceAsString();
        $client= new GearmanClient();
        $client->addServers($config['gearman']['servers']);
        $data = array(
            'message' => $message,
