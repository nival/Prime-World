...
class MirrorDaemon(ICSVActionTimingWriter, ITornadoServer):
    ...
    def tick(self):
        try:
            ...
            if options.mirror_mode == "read":

                now = time.time()

                info("read tick, len of queue delete: %d", len(self.queue_delete_requests))
                if len(self.queue_delete_requests) >= self.REMOVE_TASKS_QUEUE_LEN:
                    queue_delete = self.queue_delete_requests
                    self.I.MIRROR.deleteRequests(queue_delete, callback=mirror_info_callback)
                    self.queue_delete_requests[:] = []

                # достаем свеженькие:
                for data_type in self.data_types:
                    # только вот не нужно тащить новые запросы, если у нас переполнены исходящие очереди
                    if self.check_request_status==CheckMirrorRequestStatus.DONE:
                        try:
                            loop = tornado.ioloop.IOLoop.instance()
                            export_pending_count = len(loop.export.queries)
                            export_reply_count = len(loop.export.replies)
                            pending_count = self.pending_counts.pick(data_type)
                            pending_ts = self.pending_get_timestamps[data_type]

                           # if ((pending_count < options.mirror_read_count) and
                            if ((pending_count <= 0) and
                                (export_pending_count < options.max_pending_mysql_export) and
                                (now > pending_ts)):
                                info("read tick (mirror request status: %s, data_type: %s, pending count: %s [max %s], export queries: %s [max %s]), export replies: %s, get ts: %.3f",
                                self.check_request_status, data_type, pending_count, options.mirror_read_count, export_pending_count, options.max_pending_mysql_export,
                                export_reply_count, pending_ts)
                                self.pending_get_timestamps[data_type] = int(time.time()) + self.PENDING_GET_TIMEOUT
                                _callback = functools.partial(self.onGetMirrorRequests, data_type=data_type)
                                self.I.MIRROR.getRequests( data_type, options.mirror_service_count, options.mirror_request_type, options.mirror_read_count, options.server_id, callback=_callback )
                        except Exception:
                            catch("tick() error getting requests for data_type = %s", data_type)

                    elif self.check_request_status==CheckMirrorRequestStatus.CHECK:
                        info("tick current mirror request status %r", self.check_request_status)
                        if not self.lock_for_check_request:
                            self.lock_for_check_request = True
                            self.I.MIRROR.checkRequestStatus(options.mirror_service_count, options.mirror_request_type, data_type, self.onCheckStatus)

            elif options.mirror_mode == "write_test":
