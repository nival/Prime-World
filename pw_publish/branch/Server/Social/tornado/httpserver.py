...
class HTTPConnection(object):
    ...
    def _on_request_body(self, data):
        ...
        #info("HTTPConnection[_on_request_body] _request.body: %s", self._request.body)
        if self._request.method in ("POST", "PATCH", "PUT"):
            httputil.parse_body_arguments(
                self._request.headers.get("Content-Type", ""), data,
                self._request.body_arguments, self._request.files)

            #info("HTTPConnection[_on_request_body]: _request.body_arguments: %s", self._request.body_arguments)
...
class HTTPRequest(object):
    ...
    def __init__(self, method, uri, version="HTTP/1.0", headers=None,
        ...
        if connection and connection.xheaders:
            ...
            ip = self.headers.get(
                "X-Real-Ip", ip)
            #~if netutil.is_valid_ip(ip): ##DAN: _valid_ip() calls getaddrinfo, with unpredictable timing. "do not want" (c)
            #~    self.remote_ip = ip
