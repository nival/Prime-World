...
class KeepaliveRequest(BaseRequest):
    def __init__(self, modeldata, params, obfuscator=True):
        ...
        self.counter = self.getIntParam("counter")
        if self.counter == None:
            self.badParam("counter")
        
        
        self.onlyFriends = self.getBoolParam("onlyFriends")
        if self.onlyFriends == None:
            self.badParam("onlyFriends")
    ...
    def parse(self):    
        if not isinstance(self.arguments, BaseRequest):
            self.arguments = KeepaliveRequest(self.acc.model, self.arguments)
