...
from logic.requests.SetSiegeScoreRequest import *
from modeldata.change_applicator import ModelChangeApplicator

class Set_siege_score( SubAction, SetSiegeScoreRequest ):
    ...
    def onStart(self):
        ...
        self.I.GS.setSiegeScore(self.acc, self.arguments.targetAuid, self.arguments.score, self.onSetScore)


    @model_callback
    @action_timing
    def onSetScore(self, response):
        if response:
            info("onSetScore response: %s" % self.I.MD.pretty(response))
