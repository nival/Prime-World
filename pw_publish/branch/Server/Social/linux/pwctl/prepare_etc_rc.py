    ...
    if i != 'prepare_etc.py' and os.path.isdir(i)==False:
        with open(fromPath+i) as f:
            ...
            with open(to, "wt+") as fout:
                for line in f:
                    line = line.replace('192.168.50.2',ip)
                    line = line.replace('192.168.50.6',ip)
                    line = line.replace('/var/log/pwlobby/',wk_dir+'/logs/')
                    line = line.replace('/opt/pwcastle',wk_dir)
                    #line = line.replace('PYTHON="/usr/bin/python"','PYTHON="' + wk_dir + '/venv/bin/python"')
                    line = line.replace('PYTHON="/usr/bin/python"','PYTHON="/usr/local/bin/python"')
