    ...
    def onWebServerDead(self):
        ...
        if name and name.isalnum():
            if name in servers:
                acc = servers.get( name )
                if acc and acc.on:
                    warn("Server %r DEAD: removing from databases" % name)
                    acc.on = 0
                    acc.rdy = 0
...
class BaseCoordinatorServer( ITornadoServer, IServerStatistic ):
    ...
    def initInterfaces( classOf ):
        ...
        iDict.CHAT = IfaceFactory.makeIChatServerInterface( options.chat, coord_cfg.COORDINATOR_CHAT_SERVICES, HTTP=iDict.HTTP, \
    ...
    def onCheckAliveWS(self, response, server_name=""):
        ...
        if response:
            ...
            if acc and not acc.rdy:
                acc.rdy = 1
                acc.need_send = True
            info( "Server is alive (suid=%r[%d] user_count=%d)",server_name, server_id, userCount)
            ...
            # неживой, похоже
            warn("Server %r not responding to check!", server_name)
