...
public class FlashShapeMaterial : FlashMaterial
{
  public FlashShapeMaterial():
    base(Shader.Find( "Flash/ShapeShaderNew" ))
  {
  ...
  {
    return  (int)_BlendMode*1 + (int)_EnableWrap*3 + (int)_FillType*6 + (int)_HasColorMatrix*18 + (int)_HasScaleGrid*36 + (int)_IsMask*72 ;
  }
}
