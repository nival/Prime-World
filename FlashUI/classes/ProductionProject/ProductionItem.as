package 
{
	...
	public class ProductionItem extends MovieClip
	{
		...
		private var currentCycle:ProductionData;
		
		public function ProductionItem():void 
		{
		...
		public	function	SetCycle(cycle:ProductionData): void
		{  
			currentCycle = cycle;
			
			 title.text = cycle.TitleText;;
			 produceIcon_mc.ProduceIconInit(cycle.ProductionBonus.BounusIcon);
			 res_txt.text = cycle.ProductionBonus.Bonus.toString();
			 bathProgress_bar.StopProgress(cycle.ProductionProgress);
			 cash.CashInit(cycle.CycleCost.CostIcon, cycle.CycleCost.CostSilver);
			 
			  timeFormater.SetTime(cycle.ProductionTimer);
