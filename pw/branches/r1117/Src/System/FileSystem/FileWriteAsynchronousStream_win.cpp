...
void FileWriteAsynchronousStream::FlushAll()
{
  ...
  FileWriteBufferedStream::SetFilePosition(LODWORD(filePosition), HIDWORD(filePosition), FILE_BEGIN);
...
unsigned int FileWriteAsynchronousStream::GetSectorSize(const string & path)
{
  ...
  NI_VERIFY(result, NI_STRFMT("Function GetDiskFreeSpace returned error %d.", GetLastError()), return COMMON_SECTOR_SIZE;);
