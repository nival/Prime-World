...
namespace EditorLib.Utils
{
	...
	public class HtmlFragment
	{
		...
		public static void CopyToClipboard( string htmlFragment, string title, Uri sourceUrl )
		{
			...
			string data = GetFragment( htmlFragment, title, sourceUrl );
			Clipboard.Clear();
			Clipboard.SetText( data, TextDataFormat.Html );
		}

		public static string GetFragment( string htmlFragment)
		{
			return GetFragment(htmlFragment, null, null);
		}

		public static string GetFragment( string htmlFragment, string title, Uri sourceUrl)
		{
			if ( title == null )
...
<html><head><title>" + title + @"</TITLE><meta http-equiv=Content-Type content=""text/html; charset=utf-8""></head><body><!--StartFragment-->";
