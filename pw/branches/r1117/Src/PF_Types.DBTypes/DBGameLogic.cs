...
public class AILogicParameters : DBResource
{
	...
	[Description( "????? ?? ???????????? ???????? ???????????. ???????????? ? ??????? ?????????? ???????? respawnDelay = level * heroRespawnDelayMultiplier * reviveTimeScale - heroAdvanceRespawnTime" )]
...
public class HeroRespawnParams : DBResource
{
	...
	[Description( "??????????? ????? ??????????? ?????. ???????? level * heroRespawnDelayMultiplier * reviveTimeScale  ?? ????? ???? ?????? ?????????? ? heroMinRespawnTime" )]
