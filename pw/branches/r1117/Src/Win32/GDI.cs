...
namespace Win32
{
	...
	public abstract class GDI
	{
		...
		[DllImport( "gdi32" )]
    public static extern HANDLE CreateFont( int nHeight, int nWidth, int nEscapement, int nOrientation, int fnWeight, uint fdwItalic, uint fdwUnderline, uint fdwStrikeOut,
      uint fdwCharSet, uint fdwOutputPrecision, uint fdwClipPrecision, uint fdwQuality, uint fdwPitchAndFamily, string lpszFace );

		[DllImport( "gdi32" )]
    public static extern HANDLE CreateFontIndirect( ref LOGFONT lpLogFont );
		...
		[DllImport( "gdi32" )]
		public static extern int GetCharABCWidthsW( HDC hdc, int uFirstChar, int uLastChar, ref ABC lpabc );
    ...
    [DllImport("gdi32")]
    public static extern uint GetKerningPairs(IntPtr hdc, uint nNumPairs, [Out] KERNINGPAIR[] lpkrnpair);
