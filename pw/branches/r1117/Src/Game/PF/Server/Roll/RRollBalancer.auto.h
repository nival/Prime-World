#ifndef __R_RollBalancer_H__
#define __R_RollBalancer_H__

#include <RPC/RPC.h>
#include "RollBalancer.h"
#include "RRollInstance.auto.h"


#include "RollBalancer.h"

namespace roll
{


class RIBalancer : public IBalancer, public BaseObjectMT
{
  NI_DECLARE_REFCOUNT_CLASS_2(RIBalancer, IBalancer, BaseObjectMT);
public:
  RPC_INFO("roll::IBalancer", 0x5b0ebe6);
  
  RIBalancer() : handler(0) {}
  RIBalancer( rpc::EntityHandler* _handler, rpc::IRemoteEntity* _parent )
  :  handler(_handler)
  ,  parent(_parent)

  {

  }

  ~RIBalancer()
  {
    if( handler )
    {
      handler->OnDestruct(*this);
      handler = 0;
    }
  }
  virtual rpc::RemoteEntityInfo GetInfo() const { rpc::RemoteEntityInfo info = { handler->GetId(), { RIBalancer::ID(), RIBalancer::CRC32}, handler->GetGUID() }; return info; }
  inline bool IsUpdated() const { return handler->IsUpdated(); }
  rpc::EntityHandler* GetHandler() { return handler; }

  void LoadNotify(  RIInstance* inst, const string & serviceId, int currentLoad )
  {
    handler->Go(handler->Call( 0, inst, serviceId, currentLoad ));
  }

  template <typename T>
  rpc::ECallResult::Enum AllocateInstance( T* object, void (T::*func)(RIInstance* result) )
  {           
    rpc::Transaction* transaction = handler->Call( 1 );
    if (transaction)
    {
      transaction->RegisterAsyncCall( transaction->GetInfo(), new rpc::FunctorNoContext<T, RIInstance*>(object, func) );
      return handler->Go(transaction);
    }
    return rpc::ECallResult::NoTransaction;
  }
  

  template <typename T, typename C>
  rpc::ECallResult::Enum AllocateInstance( T* object, void (T::*func)(RIInstance* result, C context, rpc::CallStatus status), const C& context, float timeout=0.f)
  {           
    rpc::Transaction* transaction = handler->Call( 1 );
    if (transaction)
    {
      transaction->RegisterAsyncCall( transaction->GetInfo(), new rpc::FunctorContext<T, RIInstance*, C>(object, func, context), timeout );
      return handler->Go(transaction);
    }
    return rpc::ECallResult::NoTransaction;
  }
  



  bool Update(rpc::IUpdateCallback* callback=0)
  {
    return handler->Update(this, callback);
  }

  bool SetUpdateCallback(rpc::IUpdateCallback* callback=0)
  {
    return handler->SetUpdateCallback(callback);
  }

  void ReadOnly( bool val )
  {
    handler->ReadOnly( val );
  }

  void Publish()
  {
    handler->Publish();
  }
 
  StrongMT<rpc::INode> GetNode(int index) { return GetHandler()->GetNode(index); }
  StrongMT<rpc::INode> GetNode(const char* name) { return GetHandler()->GetNode(name); }
  virtual rpc::IUpdateCallback* GetUpdateCallback() { StrongMT<rpc::IRemoteEntity> _parent = parent.Lock(); return handler->GetUpdateCallback(_parent); }
  virtual void SetParent(rpc::IRemoteEntity* _parent) { parent = _parent; }
  virtual rpc::Status GetStatus() { return handler->GetStatus(); }

  static uint GetClassCrcStatic() { return 0x5b0ebe6; }
protected:
  friend class rpc::Gate;




private:
  StrongMT<rpc::EntityHandler> handler;
  WeakMT<rpc::IRemoteEntity> parent;


};

}



#endif // __R_RollBalancer_H__
