<%@ Page Title="Title" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="ViewPage<RerollShopGroupSlotsModel>" %>
...
<asp:Content ID="cLinks" ContentPlaceHolderID="Links" runat="server">
  ...
  <script type="text/javascript">
    ...
    var SlotsEnum = eval([<% ="\"" + RerollShopGroupSlotsModel.GetAllTexts(Model.Slots) + "\"" %>]);
    var SlotsEnumValues = eval([<% ="\"" + RerollShopGroupSlotsModel.GetAllValues(Model.Slots) + "\"" %>]);
    ...
    function add_dynamicblock() {
      ...
      block.find("#Group").change({ id: blockid }, function(event) {
          var slots = $("#allgroupsslots #C" + $(event.target).val()).val();
          $("#dynamicblock_" + event.data.id + " .Slots").each(function () {
               CommonScripts.AddListBox($(this), SlotsEnumValues, SlotsEnum, true);
          });
          $("#dynamicblock_" + event.data.id + " .Slots select").val(slots.split(','));          
          $("#dynamicblock_" + event.data.id + " #SlotsOld").text(slots);
      }).change();        
    ...
    function fill_dynamicblock(group, slots) {
      var id = blockid;
      add_dynamicblock();
      $("#dynamicblock_" + id + " #Group [value='" + group + "']").attr("selected", "selected").change();
      $("#dynamicblock_" + id + " .Slots").each(function () {
               CommonScripts.AddListBox($(this), SlotsEnumValues, SlotsEnum, true);
          });
      $("#dynamicblock_" + id + " .Slots select").val(slots.split(','));
      $("#dynamicblock_" + id + " .Slots select").change();
    }

    $(document).ready(function() {
      // Заполнение данными из модели (при редактировании ивента)
      <% foreach (var p in Model.NewPrices) { %>
        fill_dynamicblock('<%= p.Group %>', '<%= p.Slots %>');
      <% } %>
...
<asp:Content ID="cMain" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <h1>Reroll group slots event</h1>
  ...
  <% using (Html.BeginForm()) { %>
    ...
    <a href="javascript:add_dynamicblock()">Add new group slots</a>
  ...
  <div id="dynamicblock_prototype" style="margin-top: 10px; margin-bottom: 15px; display: none">
    ...
    <table>
      ...
      <tr>
        <td>Slots</td>
        <td class="Slots"><input type="text" id="Slots" name="Slots" /></td>
      </tr>
      <tr>
        <td></td>
        <td><span id="SlotsOld"></span> (old slots)</td>
      </tr>      
    </table>
    ...
    <div id="allgroupsslots">
        <% foreach (var p in Model.DefaultPrices) { %>
          <input type="hidden" id="C<%= p.Group %>" value="<%= p.Slots %>"/>
        <% } %>
    </div>
