    ...
    <table>
      ...
      <% foreach (var talent in Model.Talents.OrderByDescending(m => m.Name))
        ...
        <tr>
          ...
          <td><%= Html.Encode(talent.HolderAuid) %></td>
          <td><%= Html.Encode(talent.Points) %></td>
          <td><%= Html.Encode(talent.HeroClassId) %></td>
          <td><%= Html.ActionLink("Return to bank", "ReturnTalentToGuildBank", new { guildid = Model.GuildId, talentInstanceId = talent.InstanceId }, new { @class = "link-delete" })%></td>
        </tr>
      <% } %>
    </table> 
  <div>
