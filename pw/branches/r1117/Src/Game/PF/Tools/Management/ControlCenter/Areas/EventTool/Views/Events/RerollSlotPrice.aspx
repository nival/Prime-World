<%@ Page Title="Title" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="ViewPage<RerollShopSlotPriceModel>" %>
  ...
  <script type="text/javascript">
    ...
    function add_dynamicblock() {
      ...
      block.find("#Slot").change({ id: blockid }, function(event) {
        ...
        var minPriceOfSlot = $("#allslotsmin #" + $(event.target).val() + groupId).val();
        $("#dynamicblock_" + event.data.id + " #MinPrice").val(minPriceOfSlot);
        $("#dynamicblock_" + event.data.id + " #MinPriceOld").text(minPriceOfSlot);
        var maxPriceOfSlot = $("#allslotsmax #Max" + $(event.target).val() + groupId).val();
        $("#dynamicblock_" + event.data.id + " #MaxPrice").val(maxPriceOfSlot);
        $("#dynamicblock_" + event.data.id + " #MaxPriceOld").text(maxPriceOfSlot);
      });
      ...
      block.find("#Group").change({ id: blockid }, function (event) {
        ...
        selectSlot.empty();
        $("#allslotsmin input[name=" + $(event.target).val() + "]").each(function () {
    ...
    function fill_dynamicblock(group, slot, minprice, maxprice) {
      var id = blockid;
      add_dynamicblock();
    ...
    $(document).ready(function() {
      ...
      <% foreach (var p in Model.NewPrices) { %>
        fill_dynamicblock('<%= p.Group %>', '<%= p.Slot %>', <%= p.MinPrice %>, <%= p.MaxPrice %>);
      <% } %>
...
<asp:Content ID="cMain" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <div id="allslotsmin">
    <% foreach (var p in Model.DefaultPrices) { %>
      <input type="hidden" id="<%= p.Slot + p.Group%>" name="<%= p.Group %>" value="<%= p.MinPrice %>" title="<%= p.Slot %>"/>
    <% } %>
  </div>
  <div id="allslotsmax">
    <% foreach (var p in Model.DefaultPrices) { %>
      <input type="hidden" id="Max<%= p.Slot + p.Group %>" name="<%= p.Group %>" value="<%= p.MaxPrice %>"/>
    <% } %>
  </div>
