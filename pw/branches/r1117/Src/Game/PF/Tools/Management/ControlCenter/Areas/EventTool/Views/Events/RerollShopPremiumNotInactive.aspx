<%@ Page Title="Title" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="ViewPage<RerollShopPremiumNotInactiveEventModel>" %>
...
<div id="pagecontent" style="width: 70%">
  ...
  <h1>Reroll shop premium slots are always rerolled event</h1>
  ...
  <% using (Html.BeginForm()) { %>
    ...
    <%= Html.HiddenFor(m => m.IsInactiveDefault) %>
    ...
    <div class="editor-label">
      <%= Html.LabelFor(m => m.IsInactive) %>
    </div>
    <div class="editor-field">
      <%= Html.CheckBoxFor( m => m.IsInactive )%>
      <%= Html.ValidationMessageFor( m => m.IsInactive )%>
    </div>
    <div class="editor-label">
      <%= Model.IsInactiveDefault%> (default)
    </div>
