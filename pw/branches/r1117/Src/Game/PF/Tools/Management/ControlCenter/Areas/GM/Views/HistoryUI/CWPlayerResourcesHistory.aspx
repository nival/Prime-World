<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="ViewPage<CWPlayerResourcesFilterModel>" %>

<asp:Content ID="cTitle" ContentPlaceHolderID="TitleContent" runat="server">
ClanWars player resources change history
</asp:Content>
...
<h2>Guild points changes history for player (auid = <%= Model.Auid %>)</h2>
...
<% using (Html.BeginForm()) { %>
  ...
  <table style="margin-top: 10px">
    <tr>
      ...
      <th>Type</th>
      <th>Guild points change</th>
      <th>Guild points total</th>
      <th>Player points change</th>
      <th>Player points total</th>
      <th>Shop item</th>
    </tr>
    <% if (Model.Results != null) { %>
      <% foreach (var result in Model.Results) { %>
        <tr>
          ...
          <td><%= Html.Encode(result.PlayerPointsChange) %></td>
          <td><%= Html.Encode(result.PlayerPointsTotal) %></td>
  ...
  <div class="pager">
    ...
    <% for (int i = 1; i <= Model.TotalPages; i++) { %>
      <%= Html.ActionLink(i.ToString(), "CWPlayerResourcesHistory", 
