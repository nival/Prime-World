#ifndef __L_GameServerAllocatorIface_H__
#define __L_GameServerAllocatorIface_H__

#include <RPC/RPC.h>
#include <RPC/CppWrapper.h>
#include "HybridServerNaming.h"


#include "GameServerAllocatorIface.h"

namespace HybridServer
{


class LIGameServerAllocatorNotify : public rpc::ILocalEntity, public BaseObjectMT
{
  NI_DECLARE_REFCOUNT_CLASS_2(LIGameServerAllocatorNotify, rpc::ILocalEntity, BaseObjectMT);
public:
  LIGameServerAllocatorNotify():entityMap(0) {}
  LIGameServerAllocatorNotify(HybridServer::IGameServerAllocatorNotify* _localObject, rpc::IEntityMap* _entityMap, rpc::ILocalEntityFactory* _factory=0)
  :   factory(_factory)
  ,   entityMap(_entityMap)
  ,   localObject(_localObject) 
  {
    localObject->SetOwner( this );
  }

  ~LIGameServerAllocatorNotify() 
  {
    if (factory)
    {
      factory->Destroy(this);
    }
  }
  
  virtual rpc::CallResult::Enum Call(const rpc::MethodCallHeader& call, rpc::MethodCall& resultCall, rpc::Arguments& args, const byte* paramsData, int _paramsSize)
  {
    entityId = resultCall.info.header.entityId;
    
    static const rpc::MethodInfo methods[] = 
    {
        { "HybridServer::IGameServerAllocatorNotify::AllocNotify", 2, false, rpc::GetMethodCode(&HybridServer::IGameServerAllocatorNotify::AllocNotify) },
        { "HybridServer::IGameServerAllocatorNotify::FreeNotify", 1, false, rpc::GetMethodCode(&HybridServer::IGameServerAllocatorNotify::FreeNotify) },
    };
    if (call.id >= sizeof(methods)/sizeof(rpc::MethodInfo) || call.id < 0)
    {
      return rpc::CallResult::WrongMethodId;
    }
    rpc::MethodCallStack stack = rpc::FillStack(call, args, paramsData, _paramsSize, methods[call.id]);
    if (true || stack.methodCode == methods[call.id].methodCode)
    {
      if ( !stack.isValid ) return rpc::CallResult::DataCorruption;
      return vcall(call.id, resultCall, stack) ? rpc::CallResult::OK : rpc::CallResult::StackCorruption;
    }
    else
    {
      return rpc::CallResult::WrongMethodCode;
    }
  }

  virtual void Publish()
  {
    StrongMT<rpc::IEntityMap> sentityMap = entityMap.Lock();
    if (sentityMap)
    {
      sentityMap->Publish( entityId, this );
    }
  }

  bool vcall( byte method_id, rpc::MethodCall& call, rpc::MethodCallStack& stack );
  virtual void* _Get( uint classId ) { return (classId == rpc::_GetId("HybridServer::IGameServerAllocatorNotify", rpc::GeneratedType) ) ? localObject : 0; }
  virtual uint GetClassCrc() const { return 0x9efaab7; }
  static uint GetClassCrcStatic() { return 0x9efaab7; }

  virtual int GetMemberIndex( const void* ) { return -1; }


private:
  StrongMT<HybridServer::IGameServerAllocatorNotify> localObject;
  rpc::ILocalEntityFactory* factory;
  WeakMT<rpc::IEntityMap> entityMap;
  rpc::EntityId entityId;
};

}

namespace HybridServer
{


class LIGameServerAllocator : public rpc::ILocalEntity, public BaseObjectMT
{
  NI_DECLARE_REFCOUNT_CLASS_2(LIGameServerAllocator, rpc::ILocalEntity, BaseObjectMT);
public:
  LIGameServerAllocator():entityMap(0) {}
  LIGameServerAllocator(HybridServer::IGameServerAllocator* _localObject, rpc::IEntityMap* _entityMap, rpc::ILocalEntityFactory* _factory=0)
  :   factory(_factory)
  ,   entityMap(_entityMap)
  ,   localObject(_localObject) 
  {
    localObject->SetOwner( this );
  }

  ~LIGameServerAllocator() 
  {
    if (factory)
    {
      factory->Destroy(this);
    }
  }
  
  virtual rpc::CallResult::Enum Call(const rpc::MethodCallHeader& call, rpc::MethodCall& resultCall, rpc::Arguments& args, const byte* paramsData, int _paramsSize)
  {
    entityId = resultCall.info.header.entityId;
    
    static const rpc::MethodInfo methods[] = 
    {
        { "HybridServer::IGameServerAllocator::AllocateServer", 2, false, rpc::GetMethodCode(&HybridServer::IGameServerAllocator::AllocateServer) },
    };
    if (call.id >= sizeof(methods)/sizeof(rpc::MethodInfo) || call.id < 0)
    {
      return rpc::CallResult::WrongMethodId;
    }
    rpc::MethodCallStack stack = rpc::FillStack(call, args, paramsData, _paramsSize, methods[call.id]);
    if (true || stack.methodCode == methods[call.id].methodCode)
    {
      if ( !stack.isValid ) return rpc::CallResult::DataCorruption;
      return vcall(call.id, resultCall, stack) ? rpc::CallResult::OK : rpc::CallResult::StackCorruption;
    }
    else
    {
      return rpc::CallResult::WrongMethodCode;
    }
  }

  virtual void Publish()
  {
    StrongMT<rpc::IEntityMap> sentityMap = entityMap.Lock();
    if (sentityMap)
    {
      sentityMap->Publish( entityId, this );
    }
  }

  bool vcall( byte method_id, rpc::MethodCall& call, rpc::MethodCallStack& stack );
  virtual void* _Get( uint classId ) { return (classId == rpc::_GetId("HybridServer::IGameServerAllocator", rpc::GeneratedType) ) ? localObject : 0; }
  virtual uint GetClassCrc() const { return 0x2c488a6a; }
  static uint GetClassCrcStatic() { return 0x2c488a6a; }

  virtual int GetMemberIndex( const void* ) { return -1; }


private:
  StrongMT<HybridServer::IGameServerAllocator> localObject;
  rpc::ILocalEntityFactory* factory;
  WeakMT<rpc::IEntityMap> entityMap;
  rpc::EntityId entityId;
};

}



#endif // __L_GameServerAllocatorIface_H__