...
// Simple bounding box class.
{
	...
	index_box(coord_t min_max_x, coord_t min_max_y)
		:
		_min(min_max_x, min_max_y),
		_max(min_max_x, min_max_y)
	{
	}
	...
	const index_point<coord_t>& get_min() const { return _min; }
	const index_point<coord_t>& get_max() const { return _max; }
	
	coord_t	get_width() const { return _max.x - _min.x; }
	coord_t	get_height() const { return _max.y - _min.y; }
	...
	{
		if (loc.x >= _min.x && loc.x <= _max.x && loc.y >= _min.y && loc.y <= _max.y)
		{
			return true;
		}
		else
...
//data:
	index_point<coord_t>	_min;
	index_point<coord_t>	_max;
};
...
// You can bias the values up or down using grid_scale.
{
	...
	*y_cells = 1;
	if (item_count_estimate > 0)
	{
		...
		*x_cells = Clamp(*x_cells, 1, 256);
		*y_cells = Clamp(*y_cells, 1, 256);
	}
}
...
// Grid-based container for points.
{
		...
		m_bound(bound)
	{
		...
		assert(bound._min.x <= bound._max.x);
	...
	struct iterator
	{
		...
		// Point at next element in the iteration.
		{
			...
			m_current_cell_x++;
			while (m_current_cell_y <= m_query_cells._max.y)
			{
				for (;;)
				{
					if (m_current_cell_x > m_query_cells._max.x)
					{
						break;
					}
				...
				m_current_cell_x = m_query_cells._min.x;
				m_current_cell_y++;
			}

			assert(m_current_cell_x == m_query_cells._min.x);
			assert(m_current_cell_y == m_query_cells._max.y + 1);
	...
	iterator	begin(const box_t& q)
	{
		...
		it.m_index = this;
		it.m_query = q;
		it.m_query_cells._min = get_containing_cell_clamped(q._min);
		it.m_query_cells._max = get_containing_cell_clamped(q._max);

		assert(it.m_query_cells._min.x <= it.m_query_cells._max.x);
		assert(it.m_query_cells._min.y <= it.m_query_cells._max.y);

		it.m_current_cell_x = it.m_query_cells._min.x;
		it.m_current_cell_y = it.m_query_cells._min.y;
...
private:
	...
	{
		index_point<int>	ip;
		ip.x = int(((p.x - m_bound._min.x) * coord_t(m_x_cells)) / (m_bound._max.x - m_bound._min.x));
		ip.y = int(((p.y - m_bound._min.y) * coord_t(m_y_cells)) / (m_bound._max.y - m_bound._min.y));
...
// Grid-based container for boxes.
{
		...
		m_query_id(0)
	{
		...
		assert(bound._min.x <= bound._max.x);
		assert(bound._min.y <= bound._max.y);
		m_grid = new nstl::vector<grid_entry_t*>[m_x_cells * m_y_cells];
	}
	...
	~grid_index_box()
	{
		...
		// repeated many times).
		nstl::vector<grid_entry_t*>&	cell_array = m_grid[0];
	...
	struct iterator
	{
		...
		// element, and return true.
		{
			...
			nstl::vector<grid_entry_t*>*	cell_array = m_index->get_cell(m_current_cell_x, m_current_cell_y);
	...
	iterator	begin(const box_t& q)
	{
		...
		if (m_query_id == 0)
		{
			...
			{
				nstl::vector<grid_entry_t*>*	cell_array = &m_grid[i];
...
private:
  ...
  nstl::vector<grid_entry_t*>*	get_cell(int x, int y)
	{
		assert(x >= 0 && x < m_x_cells);
		assert(y >= 0 && y < m_y_cells);
	...
	// Get the indices of the cell that contains the given point.
	{
		index_box<int>	ib(get_containing_cell_clamped(p._min), get_containing_cell_clamped(p._max));
		return ib;
	}
...
//data:
	...
	int	m_y_cells;
	int	m_query_id;
	nstl::vector<grid_entry_t*>*	m_grid;
};
