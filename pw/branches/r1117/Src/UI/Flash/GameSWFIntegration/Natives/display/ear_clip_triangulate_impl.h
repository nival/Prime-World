...
struct poly_vert {
  ...
  int m_uniqueID;
	...
	poly_vert() : m_next(-1), m_prev(-1), m_state(DIRTY), m_uniqueID(-1) {}
	poly_vert(coord_t x, coord_t y, int prev, int next, int uniqueID ) : m_v(x, y), m_next(next), m_prev(prev), m_state(DIRTY), m_uniqueID(uniqueID) {}
...
//	array<coord_t>* m_results;
	...
	nstl::vector<poly_vert> m_verts;
	nstl::vector<path_info> m_input_paths;
	...
	nstl::vector<coord_t>* m_debug_edges;
};
...
struct vert_index_sorter {
  vert_index_sorter(const nstl::vector<poly_vert>& verts)
		: m_verts(verts) {
	}
  ...
  vert_index_sorter& operator=( const vert_index_sorter& p ) { m_verts = p.m_verts; return *this; }

	const nstl::vector<poly_vert>& m_verts;
};
...
{
	// Sort.
	nstl::vector<poly_vert> verts = ts->m_verts;
	nstl::vector<int> vert_indices(verts.size());   // verts[vert_indices[0]] --> leftmost vert
...
static void init(tristate* ts, trilist_accepter* output, path_supplier* input,
		 ...
		 int debug_halt_step, nstl::vector<coord_t>* debug_edges)
// Pull the paths into *tristate.
{
	...
	for (;;) {
		int coord_count = 0;
		const pair<coord_t, int>* path_data = NULL;
		...
		if (coord_count > 0) {
			...
			for (int j = 0; j < coord_count; j += 2) {
				...
				ts->m_verts.push_back(poly_vert(path_data[j].first, path_data[j + 1].first, vert_index - 1, vert_index + 1, path_data[j].second));
...
static void debug_make_x(nstl::vector<coord_t>* out, const vec2_t& v)
{
	if (!out) return;
...
static void debug_make_plus(nstl::vector<coord_t>* out, const vec2_t& v)
{
	if (!out) return;
	out->push_back(v.x);
	out->push_back(v.y - 200);
...
static void debug_make_square(nstl::vector<coord_t>* out, const vec2_t& v)
{
	if (!out) return;
	out->push_back(v.x - 200);
	out->push_back(v.y - 200);
...
static void emit_triangle(tristate* ts,
        ...
        int uniqueID1, int uniqueID2, int uniqueID3)
{
...
/* 	ts->m_results->push_back(ts->m_verts[vi2].m_v.y); */
	ts->m_output->accept_triangle( x0, y0, x1, y1, x2, y2, uniqueID1, uniqueID2, uniqueID3 );
}
	...
	while (ts->m_next_dirty < ts->m_verts.size()) {
      ...
      pvi2;
			...
			if (vi0 != vi1 && vi0 != vi2 && vi1 != vi2) {
                ...
                ts->m_verts[vi0].m_uniqueID,
                ts->m_verts[vi1].m_uniqueID,
                ts->m_verts[vi2].m_uniqueID);
			}
			return true;
		}
	}
...
static void compute_triangulation_impl(
	...
	path_supplier* input,
	int debug_halt_step,
	nstl::vector<coord_t>* debug_edges)
{
...
{
  ear_clip_array_io(nstl::vector<nstl::pair<coord_t, int>>* results, int path_count, const nstl::vector<nstl::pair<coord_t, int>> paths[])
		:
		m_results(results),
    ...
    nstl::pair<coord_t, int>* p = &m_results->back() - 5;
		p[0].first = x0;
    p[0].second = uniqueID1;
		p[1].first = y0;
    p[1].second = uniqueID1;
		p[2].first = x1;
    p[2].second = uniqueID2;
		p[3].first = y1;
    p[3].second = uniqueID2;
		p[4].first = x2;
    p[4].second = uniqueID3;
		p[5].first = y2;
    p[5].second = uniqueID3;
	}
	...
	int get_path_data(int* coord_count, pair<coord_t, int> const *path_data[])
	{
...
// data:
  nstl::vector<nstl::pair<coord_t, int>>* m_results;
	int m_path_count;
	const nstl::vector<nstl::pair<coord_t, int>>* m_paths;
	int m_next_path;
};
...
static void compute_triangulation(
	nstl::vector<pair<coord_t, int>>* results,
	int path_count,
	const nstl::vector<pair<coord_t, int>> paths[],
	int debug_halt_step,
	nstl::vector<coord_t>* debug_edges)
{
