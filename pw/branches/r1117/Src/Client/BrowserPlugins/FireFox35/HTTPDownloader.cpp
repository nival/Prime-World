...
byte *HTTPDownloader::ReadLocalFile(wchar_t const *localPath, int64 *npSize)
{
...
int64 HTTPDownloader::LocalFileSize(wchar_t const *localPath)
{
  int64 nSize = 0;
  HANDLE hFile = CreateFileW(localPath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL); 
  if(INVALID_HANDLE_VALUE == hFile)
...
std::wstring HTTPDownloader::GetHeaderField(HINTERNET hRequest, unsigned code)
{
  DWORD headerIndex  = 0;
...
HTTPDownloader::DownloadStatus HTTPDownloader::DownloadFileIfNeeded(char const *url, wchar_t const *localPath)
{
  if(!PathIsURLA(url))
...
HTTPDownloader::DownloadStatus HTTPDownloader::DownloadFile(char const *url, wchar_t const *localPath)
{
  return DownloadFile(url, localPath, L"GET");
}

HTTPDownloader::DownloadStatus HTTPDownloader::DownloadFile(char const *url, wchar_t const *localPath, wchar_t const *method, int64 *npExpectedContentSize)
{
...
bool HTTPDownloader::AcceptCertificate(TCHAR const *cpServer, INTERNET_PORT port)
{
...
bool HTTPDownloader::DownloadFileStart(char const *url, wchar_t const *localPath, wchar_t const *method)
{
  USES_CONVERSION;
  BOOL  bResults = FALSE;
  ...
  if (hConnect)    
  {
    ...
    bool bSecure = ((urlComp.dwSchemeLength == 5) && (_tcsnicmp(urlComp.lpszScheme, _T("HTTPS"), 5) == 0));
...
bool HTTPDownloader::DownloadFileReceive()
{
  DWORD dwSize;
  DWORD dwDownloaded;
  status = DL_RECIEVING;
...
void HTTPDownloader::DownloadFileFinalize()
{
  if(pBuffer) 
  {
    delete [] pBuffer; 
...
HTTPDownloader::DownloadStatus HTTPDownloader::DownloadFileHead(char const *url, wchar_t const *localPath, int64 *npExpectedContentSize)
{
