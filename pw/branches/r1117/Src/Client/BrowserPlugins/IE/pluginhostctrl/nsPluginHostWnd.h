...
#define DLL_BINARY_STR        _T("DLL_BINARY")
...
public:
    ...
    void AppendElement(Entry entry)
    {
        ...
        {
            m_nMaxSize += m_nExpandArrayBy;
			Entry* tmp = (Entry *) realloc(m_pData, sizeof(Entry) * m_nMaxSize); 
			if (tmp)
				m_pData = tmp;
        }
        ATLASSERT(m_pData);
...
DECLARE_WND_CLASS(_T("NivalPluginHostWindow"))
...
public:
    ...
    struct LoadedPluginInfo
    {
        ...
        PluginType    type;
        LoadedPluginInfo(): szFullPath(NULL), hInstance(NULL), nRefCount(0), type(PT_NA) {}
    };
...
public:
  ...
  HRESULT GetPluginVersion(/*[out, retval]*/ BSTR *pVal);
  HRESULT SetPluginVersion(/*[in]*/ BSTR newVal);
  ...
  HRESULT FindPluginPathByContentTypeInResource(const TCHAR *pszContentType, TCHAR **ppszPluginPath);
...
private:
  HMODULE ResourceLoadPlugin(const TCHAR *pszPluginPath, LoadedPluginInfo *pPlugInfo);
  bool    PreparePath(const TCHAR * path);
  HANDLE  CreateTempFile(TCHAR const * pBase, TCHAR ** pPlugPath);
