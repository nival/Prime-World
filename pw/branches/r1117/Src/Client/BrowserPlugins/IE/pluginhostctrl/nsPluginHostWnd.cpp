...
const int kArraySizeIncrement = 10;
    ...
    m_pszArgValues(NULL)
{
    ...
    m_bstrPluginVersion.Attach(SysAllocString(L"1.0.0.1"));
}

nsPluginHostWnd::~nsPluginHostWnd()
{
  BOOL bHandled = TRUE;
  OnDestroy(0, NULL, NULL, bHandled);
}
...
LRESULT nsPluginHostWnd::OnPaint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    if (m_bPluginIsWindowless && m_NPPFuncs.event)
    {
        ...
        NPRect paintRect;
        paintRect.left   = (uint16_t)rc.left;
        paintRect.top    = (uint16_t)rc.top;
        paintRect.right  = (uint16_t)rc.right;
        paintRect.bottom = (uint16_t)rc.bottom;
...
HRESULT nsPluginHostWnd::InitPluginCallbacks()
{
    ...
    g_NPNFuncs.geturl           = NPN_GetURL;
    g_NPNFuncs.posturl          = NPN_PostURL;
    g_NPNFuncs.requestread      = NPN_RequestRead;
    g_NPNFuncs.newstream        = NPN_NewStream;
    g_NPNFuncs.write            = NPN_Write;
    g_NPNFuncs.destroystream    = NPN_DestroyStream;
    g_NPNFuncs.status           = NPN_Status;
    g_NPNFuncs.uagent           = NPN_UserAgent;
    g_NPNFuncs.memalloc         = NPN_MemAlloc;
    g_NPNFuncs.memfree          = NPN_MemFree;
    g_NPNFuncs.memflush         = NPN_MemFlush;
    g_NPNFuncs.reloadplugins    = NPN_ReloadPlugins;
    g_NPNFuncs.getJavaEnv       = NPN_GetJavaEnvProcPtr(NPN_GetJavaEnv);
    g_NPNFuncs.getJavaPeer      = NPN_GetJavaPeerProcPtr(NPN_GetJavaPeer);
    g_NPNFuncs.geturlnotify     = NPN_GetURLNotify;
    g_NPNFuncs.posturlnotify    = NPN_PostURLNotify;
    g_NPNFuncs.getvalue         = NPN_GetValue;
    g_NPNFuncs.setvalue         = NPN_SetValue;
    g_NPNFuncs.invalidaterect   = NPN_InvalidateRect;
    g_NPNFuncs.invalidateregion = NPN_InvalidateRegion;
    g_NPNFuncs.forceredraw      = NPN_ForceRedraw;
...
HRESULT nsPluginHostWnd::SetPluginVersion(BSTR newVal)
{
  return S_OK;
}
...
HRESULT nsPluginHostWnd::GetPluginVersion(BSTR *pVal)
{
  if (!pVal)
  {
    return E_INVALIDARG;
  }
  *pVal = m_bstrPluginVersion.Copy();
  return S_OK;
}
...
struct FindMimeStruct
{
  HMODULE      hDll; 
  const TCHAR *pszContentType; //in
  LPTSTR       resName;        // out
};

BOOL CALLBACK nsPluginHostWnd::EnumResNameProc(HMODULE hModule, LPCTSTR lpszType, LPTSTR lpszName, LONG_PTR lParam)
{
  FindMimeStruct *pThis = (FindMimeStruct *)lParam;
  UINT16 nID = 0;
  TCHAR *cp = lpszName;
  ...
  {
    nID = (UINT16)(DWORD)lpszName;
  }

  cp = new TCHAR [100]; 
  if(0 != LoadString(pThis->hDll, nID, cp, 99))
  {
    if(_tcscmp(cp, pThis->pszContentType) == 0)
    {
      if(!IS_INTRESOURCE(lpszName))
      {
        if(pThis->resName!= NULL && !IS_INTRESOURCE(pThis->resName))
        {
          free(pThis->resName);
          pThis->resName = NULL;
        }
        pThis->resName = _tcsdup(lpszName);
      }
      else
      {
        pThis->resName = lpszName;
      }
    }
  }
  delete [] cp;
...
HRESULT nsPluginHostWnd::FindPluginPathByContentTypeInResource(const TCHAR *pszContentType, TCHAR **ppszPluginPath)
{
  HMODULE hThis;
  ...
  FindMimeStruct mime = {hThis, pszContentType, NULL};
  BOOL ret = EnumResourceNames(hThis, DLL_BINARY_STR, EnumResNameProc, (LONG_PTR)&mime);
  if(ret == 0 || mime.resName == NULL)
  {
    return E_FAIL;
  }
  //check convention for fake names
  if(mime.resName[0] != '*')
  {
    if(!IS_INTRESOURCE(mime.resName))
      free(mime.resName);
    return E_FAIL;
  }
  //just pass ownership
  *ppszPluginPath = mime.resName;
  mime.resName = NULL;
  return S_OK;
}
...
HRESULT nsPluginHostWnd::FindPluginPathByContentType(const TCHAR *pszContentType, TCHAR **ppszPluginPath)
{
    *ppszPluginPath = NULL;
    ...
    //check inside
    if(S_OK == FindPluginPathByContentTypeInResource(pszContentType, ppszPluginPath))
    {
      return S_OK;
    }
...
bool nsPluginHostWnd::PreparePath(const TCHAR * path)
{
...
HANDLE  nsPluginHostWnd::CreateTempFile(TCHAR const * pBase, TCHAR ** pPlugPath)
{
  const DWORD nBufSize = 2048;
  TCHAR *lpTempPathBuffer = new TCHAR[nBufSize];
  //  Gets the temp path env string (no guarantee it's a valid path).
  DWORD dwRetVal = GetTempPath(nBufSize - 1, lpTempPathBuffer); // buffer for path 
  if (dwRetVal > (nBufSize - 1) || dwRetVal == 0)
  {
    delete [] lpTempPathBuffer;
    return INVALID_HANDLE_VALUE;
  }
  if(!PreparePath(lpTempPathBuffer))
  {
    delete [] lpTempPathBuffer;
    return INVALID_HANDLE_VALUE;
  }
  TCHAR *lpFilePathBuffer = new TCHAR[MAX_PATH];

  // Create a temporary file. 
  UINT uRetVal = GetTempFileName(lpTempPathBuffer, pBase, 0, lpFilePathBuffer);  // buffer for name 
  delete [] lpTempPathBuffer;
  if(uRetVal == 0)
  {
    delete [] lpFilePathBuffer;
    return INVALID_HANDLE_VALUE;
  }

  HANDLE hFile = CreateFile(lpFilePathBuffer, GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_TEMPORARY, NULL); 
  if(hFile != INVALID_HANDLE_VALUE)
  {
    *pPlugPath = _tcsdup(lpFilePathBuffer);
  }
  delete [] lpFilePathBuffer;
  return hFile;
}

HMODULE nsPluginHostWnd::ResourceLoadPlugin(const TCHAR *pszPluginPath, LoadedPluginInfo *pPlugInfo )
{
  HMODULE hThis;
  BOOL bRet  = GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, (LPCTSTR)(void *)GetAddressOfThisFunction, &hThis);
  if(!bRet)
    return NULL;                  
  //skip firs one
  HRSRC hRes = FindResource(hThis, pszPluginPath, DLL_BINARY_STR);
  DWORD nSize = SizeofResource(hThis, hRes);
  HGLOBAL hGlb = LoadResource(hThis, hRes);
  LPVOID  pDllBin = LockResource(hGlb);

  //memory load library is good technique, but
  //return  (HMODULE)MemoryLoadLibrary(pDllBin);

  HANDLE hFile = CreateTempFile(pszPluginPath + 1, &pPlugInfo->szFullPath);
  DWORD nWritten;
  WriteFile(hFile, pDllBin, nSize, &nWritten, NULL);
  if(nSize != nWritten)
  {
    if(pPlugInfo->szFullPath != NULL)
    {
      free(pPlugInfo->szFullPath);
      pPlugInfo->szFullPath = NULL;
    }
    return NULL;
  }
  CloseHandle(hFile);

  TCHAR * newstr = _tcsdup(pPlugInfo->szFullPath);
  memmove(pPlugInfo->szFullPath + 1,  pPlugInfo->szFullPath, (_tcslen(newstr)) * sizeof(TCHAR));
  pPlugInfo->szFullPath[_tcslen(newstr)] = '\0';
  MoveFile(newstr, pPlugInfo->szFullPath + 1);
  free(newstr);
  pPlugInfo->szFullPath[0] = '*'; 
  HMODULE hLib = LoadLibrary(pPlugInfo->szFullPath + 1);
  if(hLib == NULL)
  {
    free(pPlugInfo->szFullPath);
    pPlugInfo->szFullPath = NULL;
  }
  m_pLoadedPlugin->type = PT_MEMORY;
  return hLib;
}
...
HRESULT nsPluginHostWnd::LoadPlugin(const TCHAR *szPluginPath)
{
    ...
    { 
      ...
      {
        TCHAR *fname = PathFindFileName(m_LoadedPlugins[i]->szFullPath + 1);
        if (_tcsncmp(fname, szPluginPath + 1, _tcslen(szPluginPath + 1)) == 0)
        {
          alreadyLoaded = true;
        }
      }
    ...
    if(szPluginPath[0] == L'*')
    {
      hInstance = ResourceLoadPlugin(szPluginPath, m_pLoadedPlugin);
    }
    else
    {
      hInstance = LoadLibrary(szPluginPath);
    }
    if (!hInstance)
    {
    ...
    if(m_pLoadedPlugin->szFullPath == NULL)
    {
      m_pLoadedPlugin->szFullPath = _tcsdup(szPluginPath);
    }
    if(m_pLoadedPlugin->type == PT_NA)
    {
      m_pLoadedPlugin->type = PT_NORMAL;
    }
...
HRESULT nsPluginHostWnd::UnloadPlugin()
{
    ...
    {
      NP_PLUGINSHUTDOWN pfnShutdown = (NP_PLUGINSHUTDOWN)GetProcAddress(m_pLoadedPlugin->hInstance,"NP_Shutdown");
      if (pfnShutdown)
      {
        pfnShutdown();
      }
      ...
      if(m_pLoadedPlugin->type == PT_MEMORY)
      {
        DeleteFile(m_pLoadedPlugin->szFullPath + 1); 
      }
