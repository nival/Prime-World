...
LRESULT nsURLDataCallback::OnNPPWrite(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    if (m_bSaveToTempFile)
    {
        if (!m_szTempFileName)
        {
            m_szTempFileName = _strdup(_tempnam(NULL, "moz"));
        }
        if (!m_pTempFile)
        {
            fopen_s(&m_pTempFile, m_szTempFileName, "wb");
        }
    ...
    if (m_pOwner->m_NPPFuncs.write &&
        m_bNotifyOnWrite)
    {
        int32_t nConsumed = m_pOwner->m_NPPFuncs.write(
            pWriteData->npp,
            pWriteData->stream,
            pWriteData->offset,
            pWriteData->len,
    ...
    /* [in] */ STGMEDIUM __RPC_FAR *pstgmed)
{
    ...
    while (m_nDataPos < dwSize)
    {
        ...
        if ((int32_t)nBytesToRead > writeReadyData.result)
        {
