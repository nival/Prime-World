...
// Apply sharpen filter a number of times, depending on current mip level
FloatImage * FloatImage::sharpenSoft(uint passes) const
{
	float f[9]  = { -1, -1, -1,  -1, 16, -1,  -1, -1, -1 };
	int dx[9] = { -1,  0, +1,  -1,  0, +1,  -1,  0, +1 };
	int dy[9] = { -1, -1, -1,   0,  0,  0,  +1, +1, +1 };

	AutoPtr<FloatImage> dst_image( clone() );

	AutoPtr<FloatImage> tmp_image( new FloatImage() );
	tmp_image->allocate( m_componentNum, m_width, m_height );

	for (uint pass = 0; pass < passes; pass++)
	{
		dst_image->copy(tmp_image.ptr());
		...
		for (uint c = 0; c < m_componentNum; c++)
		{
			for ( int y = 1; y < m_height - 1; y++ )
			{
				for ( int x = 1; x < m_width - 1; x++ )
				{
					...
					for(uint i = 0; i < 9; i++)
					{
						v += f[i] * tmp_image->pixel( x + dx[i], y + dy[i], c );
					}
					v /= 8.0f;
					dst_image->setPixel( v, x, y, c );
				}
			}
		}
	}
...
void FloatImage::copy(FloatImage* image ) const
{
	nvDebugCheck(image && image->m_mem);
	nvDebugCheck(m_width == image->m_width && m_height == image->m_height && m_componentNum == image->m_componentNum);

	memcpy(image->m_mem, m_mem, m_count * sizeof(float));
}
