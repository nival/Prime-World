...
protected:
  ...
  int  check_out_queue ();
  int  do_initiate_write_stream (ACE_Message_Block &mb);
  ...
  ACE_Message_Queue<ACE_NULL_SYNCH> out_mq_;
};
...
public:
  ...
  int write (ACE_Message_Block &mb)
    {
      return (connection_) ? connection_->initiate_write_stream(mb) : -1;
    }
