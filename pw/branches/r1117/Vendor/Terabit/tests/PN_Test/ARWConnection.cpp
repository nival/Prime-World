...
int
ARWConnection::start_asynch_read (const Buffer_Info& buf_info, size_t bytes)
{
    return this->stream_.read (buf_info, bytes);
}
int
ARWConnection::start_asynch_write (const Buffer_Info& buf_info, size_t bytes)
{
    return this->stream_.write (buf_info, bytes);
}
