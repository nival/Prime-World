...
{
  ACE_START_TEST (ACE_TEXT ("P1_Test"));
...
//              ACE_FD_SETSIZE));
  ...
  // let assume we do not have a collection of sessions  
  // s_manager.cancel ();
  // r_manager.cancel ();
  ...
  ACE_DEBUG ((LM_DEBUG,
              ACE_TEXT ("(%t) Proactor Cancel All AIO\n")));

  for (u_int i=0; i < cfg.demultiplexors (); ++i)
  {
    task1.get_proactor(i)->cancel_all_aio();
  }
  ...
  //ACE_Time_Value ztv = ACE_Time_Value::zero;            
  //task1.proactor()->handle_events(ztv);
