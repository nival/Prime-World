...
TestCfg::set_proactor_type (const ACE_TCHAR *ptype)
{
  ...
  switch (::toupper (*ptype))
    {
    case 'D':  this->proactor_type_ = DEFAULT; break;
    case 'A':  this->proactor_type_ = AIOCB;   break;
    case 'I':  this->proactor_type_ = SIG;     break;
    case 'E':  this->proactor_type_ = SELECT;  break;
    case 'P':  this->proactor_type_ = POLL;    break;
    //case 'R':  this->proactor_type_ = LINUXRT; break;
    case 'L':  this->proactor_type_ = LINUX_NAIO;   break;
    case 'S':  this->proactor_type_ = SUN;     break;
    case 'V':  this->proactor_type_ = DEVPOLL; break;
    case 'O':  this->proactor_type_ = EPOLL;   break;
    case 'C':  this->proactor_type_ = CB;      break;
    case 'K':  this->proactor_type_ = SUNPORT; break;
    case 'Q':  this->proactor_type_ = KQUEUE;  break;
    case 'Y':  this->proactor_type_ = DUMMY;   break;
    default:   return -1;
    }
  return 0;
}
...
void 
TestCfg::get_binding_address (TRB_Sock_Addr& addr,
                              int index)
{
    ...
    {
        char buf [512];
        ACE_OS::sprintf (buf, "%s.LOCAL-%d", host, index);
        ACE_UNIX_Addr tmp (buf);
        addr = tmp;
        ACE_OS::unlink (buf);
    }
    else
#endif
    {
        ACE_INET_Addr tmp;  // INADDR_ANY
        addr = tmp;
    }
}

void 
TestCfg::get_listen_address (TRB_Sock_Addr& listenAddr)
{
    u_short listenPort  = this->listen_port ();
    const char* host    = this->connect_host ();
...
void 
TestCfg::get_connect_address (TRB_Sock_Addr& connectAddr)
{
...
{
    get_listen_address (listenAddr);
    get_connect_address (connectAddr);
}
