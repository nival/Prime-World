...
Receiver::open (void *)
{
  ...
  flg_mask_ = ACE_Event_Handler::READ_MASK ;
...
Receiver::initiate_io (ACE_Reactor_Mask mask)
{
  ...
  if (ACE_BIT_CMP_MASK(flg_mask_, mask, mask))
    return 0;
...
int
Receiver::handle_close (ACE_HANDLE handle, ACE_Reactor_Mask mask)
{
    {
        ACE_Guard<ACE_Recursive_Thread_Mutex> locker (mutex_);

        ACE_CLR_BITS (this->flg_mask_, mask);
        if (flg_mask_ != ACE_Event_Handler::NULL_MASK)
        {
            return 0;
        }
...
int
Sender::handle_close (ACE_HANDLE handle, ACE_Reactor_Mask mask)
{
    {
        ACE_Guard<ACE_Recursive_Thread_Mutex> locker (mutex_);
...
{
  ACE_START_TEST (ACE_TEXT ("T_Test"));
