...
namespace WeifenLuo.WinFormsUI.Docking
{
	internal class VS2005DockPaneStrip : DockPaneStripBase
	{
		...
		private int GetMaxTabWidth_Document(int index)
		{
			...
			Size sizeText = TextRenderer.MeasureText(content.DockHandler.TabText, BoldFont, new Size(DocumentTabMaxWidth, height),
			                                         DocumentTextFormat);

			// CLOSE BUTTON CODE HERE 
			if (DockPane.DockPanel.ShowDocumentIcon)
				return sizeText.Width + DocumentIconWidth + DocumentIconGapLeft*2 + DocumentIconGapRight + DocumentTextGapRight +
				       DocumentIconWidth;
			else
				return sizeText.Width + DocumentIconGapLeft*2 + DocumentTextGapRight + DocumentIconWidth;
		}
		...
		private GraphicsPath GetTabOutline_Document(Tab tab, bool rtlTransform, bool toScreen, bool full)
		{
			...
			if (tab.Content == DockPane.ActiveContent || full || Tabs.IndexOf(tab) == FirstDisplayingTab)
			{
				if (RightToLeft == RightToLeft.Yes)
				{
					...
					{
						GraphicsPath.AddLine(rect.Right, rect.Bottom, rect.Right + rect.Height/2, rect.Bottom);
						GraphicsPath.AddLine(rect.Right + rect.Height/2, rect.Bottom, rect.Right - rect.Height/2 + curveSize/2,
			...
			else
			{
				if (RightToLeft == RightToLeft.Yes)
				{
					...
					else
					{
						...
						GraphicsPath.AddLine(rect.Right, rect.Bottom - rect.Height/2, rect.Right - rect.Height/2 + curveSize/2,
						                     rect.Top + curveSize/2);
					}
				}
				else
		...
		private void DrawTab_Document(Graphics g, TabVS2005 tab, Rectangle rect)
		{
			...
			Rectangle rectIconClose = rectIcon;
			rectIconClose.X = rectText.X + rectText.Width - DocumentIconWidth;
			//--		
			...
			if (DockPane.ActiveContent == tab.Content)
			{
				...
				LinearGradientMode gradientMode =
					DockPane.DockPanel.Skin.DockPaneStripSkin.DocumentGradient.ActiveTabGradient.LinearGradientMode;
			...
			g.DrawLine(p, rectIconClose.X + crop, rectIconClose.Y + crop, rectIconClose.X + rectIconClose.Width - crop,
			           rectIconClose.Y + rectIconClose.Height - crop);
			g.DrawLine(p, rectIconClose.X + crop, rectIconClose.Y + rectIconClose.Height - crop,
			           rectIconClose.X + rectIconClose.Width - crop,
			           rectIconClose.Y + crop);
		...
		protected internal override bool HitTestClose(int ind, Point ptMouse )
		{
			Rectangle r= GetTabRectangle(ind);
			if( ptMouse.Y>r.Y &&
				  ptMouse.Y<(r.Y+r.Height) &&
				  ptMouse.X<(r.X+r.Width) &&
				  ptMouse.X>(r.X+r.Width-15)) return true;
