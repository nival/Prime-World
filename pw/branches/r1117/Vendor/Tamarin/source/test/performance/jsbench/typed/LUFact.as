  ...
  var ops:Number, total:Number, norma:Number, normx:Number;

  var resid:Number, time:Number;
  ...
  function JGFsetsize(lsize):void {
    size = lsize;
  }
  ...
  function JGFinitialise():void {
    ...
    a = new Vector.<Vector.<Number>>(ldaa);
    for(var i:int=0;i<ldaa;i++)
    {
      a[i] = new Vector.<Number>(lda);
    }
    b = new Vector.<Number>(ldaa);
    x = new Vector.<Number>(ldaa);
    ipvt = new Vector.<int>(ldaa);

    var nl:int = n; // avoid integer overflow
  ...
  function matgen(a:Vector.<Vector.<Number>>, lda:int, n:int, b:Vector.<Number>):Number {
    var norma:Number;
    var init:int, i:int, j:int;
  ...
  function dgefa(a:Vector.<Vector.<Number>>,  lda:int,  n:int,  ipvt:Vector.<int>) {
    var col_k:Vector.<Number>, col_j:Vector.<Number>;
    var t:Number;
    var j:int, k:int, kp1:int, l:int, nm1:int;
    var info:int;
  ...
  function dgesl(a:Vector.<Vector.<Number>>,  lda:int,  n:int,  ipvt:Vector.<int>,  b:Vector.<Number>, job:int):void {
    var t:Number;
    var k:int, kb:int, l:int, nm1:int, kp1:int;
   ...
   */
  function daxpy( n:int,  da:Number,  dx:Vector.<Number>,  dx_off:int,  incx:int, dy:Vector.<Number>,  dy_off:int,  incy:int):void {
   ...
   */
  function ddot( n:int,  dx:Vector.<Number>,  dx_off:int,  incx:int,  dy:Vector.<Number>,  dy_off:int,  incy:int):Number {
    var dtemp:Number;
    var i:int, ix:int, iy:int;
   ...
   */
  function dscal( n:int,  da:Number,  dx:Vector.<Number>,  dx_off:int,  incx:int):void {
    var i:int, nincx:int;
   ...
   * linpack, 3/11/78.
   */
  function idamax( n:int,  dx:Vector.<Number>,  dx_off:int,  incx:int):int {
    var dmax:Number, dtemp:Number;
    var i:int, ix:int, itemp:int = 0;
