...
datasizes[3] = 100000;
  ...
  var plain1:Vector.<int>; // byte Buffer for plaintext data.

  var crypt1:Vector.<int>; // byte Buffer for encrypted data.

  var plain2:Vector.<int>; // byte Buffer for decrypted data.

  var userkey:Vector.<int>; // short Key for encryption/decryption.

  var Z:Vector.<int>; // int Encryption subkey (userkey derived).
  ...
  function buildTestData():void {
    ...
    plain1 = new Vector.<int>(array_rows);
    crypt1 = new Vector.<int>(array_rows);
    plain2 = new Vector.<int>(array_rows);
    ...
    userkey = new Vector.<int>(8,true); // User key has 8 16-bit shorts.
    Z = new Vector.<int>(52,true); // Encryption subkey (user key derived).
    DK = new Vector.<int>(52,true); // Decryption subkey (user key derived).
  ...
  function calcEncryptKey():void {
    ...
    for (var i:int = 0; i < 8; i++) // First 8 subkeys are userkey itself.
    {
  ...
  function calcDecryptKey():void {
    var j:int, k:int; // Index counters.
    var t1:int, t2:int, t3:int; // Temps to hold decrypt subkeys.
  ...
  function cipher_idea(text1:Vector.<int>, text2:Vector.<int>, key:Vector.<int>):void

  {

    var i1:int = 0; // Index into first text array.
    var i2:int = 0; // Index varo second text array.
    var ik:int; // Index varo key array.
    var x1:int, x2:int, x3:int, x4:int, t1:int, t2:int; // Four "16-bit" blocks, two temps.
  ...
  function mul(a:int, b:int):int{
    var p:int; // Large enough to catch 16-bit multiply
  ...
  function printarray(_name:String,_data):void
  {
