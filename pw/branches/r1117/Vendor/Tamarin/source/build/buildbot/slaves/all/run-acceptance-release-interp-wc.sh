...
##
if [ ! -e "$buildsdir/$change-${changeid}/$platform/$shell_release_wordcode" ]; then
    echo "Download AVMSHELL"
    ../all/util-download.sh $vmbuilds/$branch/$change-${changeid}/$platform/$shell_release_wordcode $buildsdir/$change-${changeid}/$platform/$shell_release_wordcode
    ret=$?
    test "$ret" = "0" || {
        echo "Downloading of $shell_release_wordcode failed"
        rm -f $buildsdir/$change-${changeid}/$platform/$shell_release_wordcode
        exit 1
    }
    chmod +x $buildsdir/$change-${changeid}/$platform/$shell_release_wordcode
fi
...
export AVM=$buildsdir/$change-${changeid}/$platform/$shell_release_wordcode
...
echo "message: $py ./runtests.py --vmargs=$interp   --notimecheck"
$py ./runtests.py --vmargs=$interp   --notimecheck
