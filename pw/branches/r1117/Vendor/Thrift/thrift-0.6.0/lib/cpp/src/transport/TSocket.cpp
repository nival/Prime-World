...
uint32_t TSocket::write_partial(const uint8_t* buf, uint32_t len) {
  ...
  int b = send(socket_, (const char*) buf + sent, len - sent, flags);
  ++g_socket_syscalls;
  ...
  if (b < 0) {
    ...
    if (errno_copy == EPIPE || errno_copy == WSAECONNRESET || errno_copy == WSAENOTCONN) {
      close();
...
void TSocket::setLinger(bool on, int linger) {
  ...
  int ret = setsockopt(socket_, SOL_SOCKET, SO_LINGER, (const char*) &l, sizeof(l));
  if (ret == -1) {
...
void TSocket::setNoDelay(bool noDelay) {
  ...
  int v = noDelay_ ? 1 : 0;
  int ret = setsockopt(socket_, IPPROTO_TCP, TCP_NODELAY, (const char*)&v, sizeof(v));
  if (ret == -1) {
    int errno_copy = errno;  // Copy errno because we're allocating memory.
...
void TSocket::setRecvTimeout(int ms) {
  ...
  int ret = setsockopt(socket_, SOL_SOCKET, SO_RCVTIMEO, (const char*) &r, sizeof(r));
  if (ret == -1) {
    int errno_copy = errno;  // Copy errno because we're allocating memory.
...
void TSocket::setSendTimeout(int ms) {
  ...
  int ret = setsockopt(socket_, SOL_SOCKET, SO_SNDTIMEO, (const char*) &s, sizeof(s));
  if (ret == -1) {
    int errno_copy = errno;  // Copy errno because we're allocating memory.
