...
void TServerSocket::listen() {
  int sv[2];
  if (-1 == socketpair(AF_INET, SOCK_STREAM, IPPROTO_TCP, sv)) {
  ...
  if (tcpSendBuffer_ > 0) {
    if (-1 == setsockopt(serverSocket_, SOL_SOCKET, SO_SNDBUF,
                         (const char*)&tcpSendBuffer_, sizeof(tcpSendBuffer_))) {
      int errno_copy = errno;
  ...
  if (tcpRecvBuffer_ > 0) {
    if (-1 == setsockopt(serverSocket_, SOL_SOCKET, SO_RCVBUF,
                         (const char*)&tcpRecvBuffer_, sizeof(tcpRecvBuffer_))) {
      int errno_copy = errno;
  ...
  while (true) {
    ...
    if (ret < 0) {
      ...
      if (intSock2_ >= 0 && (fds[1].revents & POLLIN)) {
        int8_t buf;
        if (-1 == recv(intSock2_, (char*)&buf, sizeof(int8_t), 0)) {
...
void TServerSocket::interrupt() {
  if (intSock1_ >= 0) {
    int8_t byte = 0;
    if (-1 == send(intSock1_, (const char*)&byte, sizeof(int8_t), 0)) {
