...
var incomingGraph = new _Graph(40);
var outgoingGraph = new _Graph(40);
...
function onPatcherStart() {
    ...
    patchManager.addManifestRequiredCompleteEventHandler(function(info) {
        ...
        rootManifest = patchManager.manifestById(host.idFromString(rootManifestUrl));
    ...
    patchManager.addPatchProgressEventHandler(function(info) {
        ...
        if (info.percent >= 0) {
            ...
            updateGraphs(0, writeRates.average);
        } else {
...
function updateGraphs(incomingSpeed, outgoingSpeed) {
    incomingGraph.addValue(incomingSpeed);
    outgoingGraph.addValue(outgoingSpeed);
}
...
function initializeTabs() {
    incomingGraph.create(document.getElementById("incomingGraph"));
    outgoingGraph.create(document.getElementById("outgoingGraph"));
