...
function App(instanceId)
{
   ...
   */
   ...
   /*!
   * This event is triggered whenever a http authentication is required.
   * \type event
   * \tparam int type type of authentication.
   * \tparam string host host for authentication.
   * \tparam string realm realm for authentication.
   * \tparam string username last username attempted.
   * \tparam string password last password attempted.
   * \return string: json object continaing the username, password and cancel keys.
   */
   this.onWillAuthenticateWebGetTarget = null;
}
...
/*!
* gets the path to file for self update
* \type string
* \returns path to file for self update
*/
App.prototype.getRestartPatchFile = function(){
   var methodBinding = {
      "method":"getRestartPatchFile"
   };
   return Interop.invoke(this.instanceId, methodBinding);
};

/*!
* sets the path to file for self update
* \tparam string value path to file for self update
*/
App.prototype.setRestartPatchFile = function(value){
   var methodBinding = {
      "method":"setRestartPatchFile", 
      "value":value
   };
   return Interop.invoke(this.instanceId, methodBinding);
};
...
/*!
* gets the path to output directory for self update
* \type string
* \returns path to output directory for self update
*/
App.prototype.getRestartPatchDirectory = function(){
   var methodBinding = {
      "method":"getRestartPatchDirectory"
   };
   return Interop.invoke(this.instanceId, methodBinding);
};

/*!
* sets the path to output directory for self update
* \tparam string value path to output directory for self update
*/
App.prototype.setRestartPatchDirectory = function(value){
   var methodBinding = {
      "method":"setRestartPatchDirectory", 
      "value":value
   };
   return Interop.invoke(this.instanceId, methodBinding);
};
...
* \tparam string utf8string UTF-8 string to be entitized.
* \type string
...
* \tparam string entitizedString Entitized string to be converted to UTF-8.
* \type string
* \returns utf8 string.
*/
...
* \tparam int type Type of credentail.
* \tparam string host Host for the credentail.
* \tparam string realm Realm for the credentail.
* \type string
* \returns json object containing username and password.
*/
App.prototype.getCredential = function(type, host, realm)
{
   var methodBinding = {
...
* \tparam string host Host for the credentail.
* \tparam string realm Realm for the credentail.
* \tparam string username Username for the credentail.
* \tparam string password Password for the credentail.
*/
App.prototype.setCredential = function(type, host, realm, username, password)
{
   var methodBinding = {
