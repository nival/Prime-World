...
function authenticateCancelClick() {
    authenticate.cancel();
}

function authenticateOkClick() {
    var username = document.getElementById("authenticateUsername").value;
    var password = document.getElementById("authenticatePassword").value;

    authenticate.ok(username, password);
}

function onAuthenticateHide() {
    document.getElementById("bodyAuthenticate").style.display = "none";
    document.getElementById("bodyAuthenticate").style.visibility = "hidden";
    document.getElementById("bodyBackground").style.display = "block";
    document.getElementById("bodyBackground").style.visibility = "visible";
}

function onAuthenticateShow(info) {
    document.getElementById("bodyAuthenticate").style.display = "block";
    document.getElementById("bodyAuthenticate").style.visibility = "visible";
    document.getElementById("bodyBackground").style.display = "none";
    document.getElementById("bodyBackground").style.visibility = "hidden";
    ...
    document.getElementById("authenticateServer").innerHTML = hostname;
    document.getElementById("authenticatePort").innerHTML = port;
    document.getElementById("authenticateRealm").innerHTML = info.realm;

    document.getElementById("authenticateUsername").value = info.userCredential.username;
    document.getElementById("authenticatePassword").value = info.userCredential.password;
}
...
function addPatchManifest(manifestUrl) {
    ...
    manifestComplete = function(info) {
        ...
        if ((typeof repair == 'undefined') || repair === null || repair.verifying === true) {
            return;
        }
...
function onPatcherStart() {
    ...
    downloadManager.addDownloadProgressEventHandler(function(info) {
        var failureReason;
        var state = info.download.getState();
        ...
        switch (state) {
            ...
            case downloadState.DOWNLOADSTALLED:
                ...
                if (failureReason > downloadFailure.NONE) {
                    ...
                    updateStatusText(getLanguageString("DownloadState_" + downloadState.nameFromId(state)));
                }
                return;
        }
    ...
    patchManager.addPatchFileErrorEventHandler(function(info) {
        updateStatusText(getLanguageString("FileError_" + osError.nameFromId(info.error)));
        
        if (osError.isUnknown(info.error) == true) {
...
function onBundlesLoaded() {
    ...
    /* This test is for non-elevated registry values
    var launcher = createLauncher();
    alert(launcher.getRegistryString('machine', 'SOFTWARE\\Microsoft\\DevDiv\\VC\\Servicing\\9.0\\RED\\1033', 'SPName', ''));    
    */
}
...
function onSkinLoaded() {
    ...
    authenticate.attach();
    authenticate.onShow = onAuthenticateShow;
    authenticate.onHide = onAuthenticateHide;
    
    repair.onProgress = onProgress;
    repair.onComplete = onRepairComplete;
	...
	host.app.info("WebBrowser: " + host.app.expandString("{Host}") + " version " + host.app.expandString("{HostVersion}"));
	host.app.info("System: " + host.app.expandString("{SystemName}") + " version " + host.app.expandString("{SystemVersion}"));
	...
	*/
    ...
    // Wire up all the visible items
    document.getElementById("closeButton").onmouseover = function() { closeHoverEnter(this); };
    document.getElementById("closeButton").onmouseout = function() { closeHoverExit(this); };
    document.getElementById("closeButton").onclick = function() { closeClick(this) };
    
    document.getElementById("minimizeButton").onmouseover = function() { minimizeHoverEnter(this); };
    document.getElementById("minimizeButton").onmouseout = function() { minimizeHoverExit(this); };
    document.getElementById("minimizeButton").onclick = function() { minimizeClick(this); };

    document.getElementById("moveWindowArea").onmousedown = function() { beginMoveWindow(this); };
    
    document.getElementById("mainTabHeader").onmouseover = function() { tabHoverEnter(this); };
    document.getElementById("mainTabHeader").onmouseout = function() { tabHoverExit(this); };
    document.getElementById("mainTabHeader").onmousedown = function() { tabSelect(this); };
    
    document.getElementById("performanceTabHeader").onmouseover = function() { tabHoverEnter(this); };
    document.getElementById("performanceTabHeader").onmouseout = function() { tabHoverExit(this); };
    document.getElementById("performanceTabHeader").onmousedown = function() { tabSelect(this); };
    
    document.getElementById("optionsTabHeader").onmouseover = function() { tabHoverEnter(this); };
    document.getElementById("optionsTabHeader").onmouseout = function() { tabHoverExit(this); };
    document.getElementById("optionsTabHeader").onmousedown = function() { tabSelect(this); };
    
    // this calls show in case the early show wasn't used
    ...
    // This tests the audio element (HTML5)
    // doAudioTest();
...
function updatePauseResumeButtons(play) {
...
function onRepairComplete(info) {
    updateCompleteStatus(info.successful, info.lastError);
...
function doAudioTest() {
    // This is a really bad way to implement normal audio!!!
    // Ideally, you want to limit the number of Audio objects created because they are expensive
    // and cef doesn't garbage collect them well.  It's a better solution to create one object
    // and change the src element to play a new sound.
    var audioTest = new Audio("audiotest.ogg");
    audioTest.play();
}
