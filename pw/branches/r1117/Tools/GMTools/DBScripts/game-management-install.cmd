...
for /d %%f in (create_gm_databases.sql) do (
...
psql -h %GMADMINDB_HOST% -p %GMADMINDB_PORT% -d postgres -U %GMDB_ADMIN_USR% -f create_gm_databases.sql
if errorlevel 1 goto failed

rem Create all tables, constraints and grant rights on GM database
psql -h %GMADMINDB_HOST% -p %GMADMINDB_PORT% -d %GMADMINDB_NAME% -U %GMDB_ADMIN_USR% -f ..\..\..\Src\Game\PF\Tools\Management\DBScripts\gmdb_admin.sql
if errorlevel 1 goto failed
