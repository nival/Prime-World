...
:: Drop fail marker
if exist logs\.failed-sstat-%LOCATION%-cont del logs\.failed-sstat-%LOCATION%-cont
...
:: Run management job 
set logfile=%~dp0logs\transformations_%mydate%_%mytime%_sstat_%LOCATION%_cont.log
call %kettlepath%\kitchen.bat /file:%~dp0/job-continuous-sstat.kjb /level:Detailed /param:analysis_host=%ANALYSISDB_HOST% /param:analysis_port=%ANALYSISDB_PORT% /param:analysis_dbname=%ANALYSISDB_NAME% /param:analysis_usr=%ANALYSISDB_ADMIN_USR% /param:analysis_pwd=%ANALYSISDB_ADMIN_PWD% /param:logs_host=%SERVICEDB_HOST% /param:logs_port=%SERVICEDB_PORT% /param:logs_dbname=%LOGSDB_NAME% /param:logs_usr=%LOGSDB_ADMIN_USR% /param:logs_pwd=%LOGSDB_ADMIN_PWD% /param:manage_host=%SERVICEDB_HOST% /param:manage_port=%SERVICEDB_PORT% /param:manage_dbname=%MANAGEMENTDB_NAME% /param:manage_usr=%REPORTING_USR% /param:manage_pwd=%REPORTING_PWD% /param:location=%LOCATION% /logfile:%logfile%

@echo.
@if errorlevel 1 (
  echo !!! FAILED !!!
  echo %logfile% > logs\.failed-sstat-%LOCATION%-cont
)