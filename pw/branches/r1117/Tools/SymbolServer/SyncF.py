...
def mkDirCall_Dummy(dirName):
    return 0
def rmvDirCall_Dummy(dirName):
    return 0
def mkFileCall_Dummy(fName):
    return 0
def rmvFileCall_Dummy(fName):
    return 0
def updFileCall_Dummy(fName):
    return 0
DummyList = [mkDirCall_Dummy, rmvDirCall_Dummy, mkFileCall_Dummy, rmvFileCall_Dummy, updFileCall_Dummy]
PrintMessages = False

class SyncFolders:
    def __init__(self, sourceDir, targetDir, recursive = True, purge = True, ignore = [], callBackList = DummyList):
        ...
        self.__mkDirCall = callBackList[0]   
        self.__rmvDirCall= callBackList[1]
        self.__mkFileCall= callBackList[2]       
        self.__rmvFileCall= callBackList[3]        
        self.__updFileCall= callBackList[4]
    ...
    def __dowork(self, sourceDir, targetDir, copyfunc, updatefunc):
        ...
        for f in dcmp.left_only:
            ...
            if stat.S_ISREG(st.st_mode):
                ...
                if self.__createDirs:
                    try:
                        os.makedirs(fulld2)
                        #@Matt here can be the problem with upper level directories (if they doesn't exist)
                        self.__mkDirCall(fulld2)
                    except Exception, e:
                        print e
        ...
        if len(os.listdir(targetDir)) == 0:
            try:
                if self.__rmvDirCall(targetDir) == 0:
    ...
    def __copy(self, fileName, sourceDir, targetDir):
        ...
        try:
            ...
            sourcefile = os.path.join(targetDir, fileName)
            self.__mkFileCall(sourcefile)
...
#some words about callBackList:
#in your list should be 5 functions such as in DummyList, each function takes string and all remove functions should returns number
#remove callbacks are called before removing files, mk callbacks are called after making file\dir
#if your callback removes file\dir by itself (inside callback) it should return 1, so this function won't try to remove this file\dir again
#and vice versa - if you want this function to remove file\dir, your callback should return 0
def SyncF(sourceFolder, targetFolder, recursive, purge, ignore, callBackList=DummyList):
    copier = SyncFolders(sourceFolder, targetFolder, recursive, purge, ignore, callBackList);
    copier.run();
