/*
#about Удаляем все неиспользуемые UITexture
...
namespace Scripts
{
  public class DeleteUnusedUITextures
  {
    ...
    private bool Work( IOperationContext context )
    {
      ...
      ObjectsSelector.CollectObjects( application, "/UI/", typeof( UITexture ), true, false, out dbids );

      for ( int i = 0; i < dbids.Count; ++i )
      {
        context.Progress( dbids.ToString(), ( i * 100 ) / dbids.Count );

        DBPtr<UITexture> ptr = DataBase.Get<UITexture>( dbids[i] );
        if ( DBID.IsNullOrEmpty( ptr.DBId ) || ptr.DBId.IsInlined )
          continue;

        var backLinks = ptr.GetBackLinks();

        //We don't have links to this UITexture
        if ( backLinks.Count == 0 )
        {
          UITexture texture = ptr.Get();
          if ( texture == null )
            continue;
