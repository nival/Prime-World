...
namespace Scripts
{
  ...
  {
    private const string TARGET_NODE_NAME = "move";//название ноды, линки в которую надо править
    private const string FROM_NODE_NAME = "";//название ноды, линки из которой надо править ( "" - из всех )
    private const float LINK_DURATION = 0.1f;
        ...
        foreach ( var node in animGraphCreator.macroNodes )
        {
          if ( string.IsNullOrEmpty( FROM_NODE_NAME ) || node.nodeName.IndexOf( FROM_NODE_NAME, System.StringComparison.InvariantCultureIgnoreCase ) >= 0 )
          {
            for ( int i = 0; i < node.simpleTransactions.Count; ++i )
            {
              ...
              {
                link.parameters.duration = LINK_DURATION;
                linksProcessed++;
