...
class Visitor(object):
    ...
    def __call__(self, file_name):
        ...
        text = fh.read()
        fh.close()
        pattern = re.compile(r'[\r\n\t]*<BasicMaterial>(.*?)</BasicMaterial>', re.DOTALL | re.MULTILINE)
        mp = MaterialParser()
...
#	out_tree['lightingIntencity'] = Node('lightingIntencity')
#	out_tree['lightingIntencity'].data = '1'
...
def addNode(name, data):
    node = Node(name)
    node.data = data
    return node

def changeLightMode(node_name, in_tree, out_tree):
    v = out_tree[node_name].data 
    if v == 'Diffuse' or v == 'DiffuseOpacity':
      out_tree['EmissivePinValue'] = addNode('EmissivePinValue', 'EmissiveOff')
      out_tree['specularPower'].data = '0'
    elif v == 'DiffuseSpecular' or v == 'DiffuseSpecularOpacity':
      out_tree['EmissivePinValue'] = addNode('EmissivePinValue', 'EmissiveOff')
    elif v == 'DiffuseEmissive':
      out_tree['EmissivePinValue'] = addNode('EmissivePinValue', 'EmissiveFromAlpha')
      out_tree['specularPower'].data = '0'
    elif v == 'DiffuseSpecularEmissive':
      out_tree['EmissivePinValue'] = addNode('EmissivePinValue', 'EmissiveFromAlpha')
    elif v == 'DiffuseSpecularToEmissive':
      out_tree['EmissivePinValue'] = addNode('EmissivePinValue', 'EmissiveFromSpecular')
    else:
      print 'ERROR! Unexpected value ', v
    del out_tree[node_name]

register('LightModePinValue', changeLightMode)
