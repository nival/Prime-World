...
class Visitor(object):
    ...
    def __call__(self, file_name):
        ...
        text = fh.read()
        fh.close()
        pattern = re.compile(r'[\n\t]*<StatsContainer>(.*?)</StatsContainer>', re.DOTALL | re.MULTILINE)
        mp = MaterialParser()
...
def addFields(in_tree, out_tree):
    out_tree['ReflectionMap'] = Node('ReflectionMap')
    out_tree['ReflectionMap']['samplerIndex'] = Node('samplerIndex')
    out_tree['ReflectionMap']['samplerIndex'].data = '3'

    out_tree['ambientColor'] = addColorNode('ambientColor', '1', '1', '1', '1')
    out_tree['diffuse1Color'] = addColorNode('diffuse1Color', '1', '1', '1', '1')
    out_tree['diffuse2Color'] = addColorNode('diffuse2Color', '1', '1', '1', '1')
    
    
def statId(node_name, in_tree, out_tree):
    statId = "Life", "Energy", "Power", "Range", "EnergyDefence", "SubstanceDefence", "MoveSpeed", "AttackSpeed", "CriticalChance", "CriticalMultiplier", "LifeDrain", "EnergyDrain", "Evasion", "LifeRegeneration", "EnergyRegeneration", "Armor", "AbilityProbability", "GainedExperienceMultiplier"
    for i in out_tree['stats']:
	for n, ii in enumerate(i['Item']):
	        for st in ii['statId']:
	            st.data = statId[n]

register('stats', statId)
