...
def collectDirs(path, patterns, ignored = [], recursive=True, add_root=True):
    result = []
    for root, dirs, files in os.walk(path):
        for name in dirs:
            relativePath = os.path.join( root, name )[len(path)+1:]
...
def runSubprocess( args , passRetCode = False):
    ...
    if os.name == 'nt':
      ...
      # Don't display the Windows GPF dialog if the invoked program dies. - See more at: #
      import ctypes
    ...
    try:
      child = subprocess.Popen(
        ...
        startupinfo=startupinfo, creationflags=subprocess_flags)
      ...
      textout = child.stdout.read()
      texterr = child.stderr.read()
