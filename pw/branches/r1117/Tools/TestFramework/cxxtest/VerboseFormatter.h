#ifndef __cxxtest__VerboseFormatter_h__
#define __cxxtest__VerboseFormatter_h__

//
// The VerboseFormatter is a TestListener that
...
namespace CxxTest
{
    ...
    class VerboseFormatter : public TestListener
    {
    public:
        VerboseFormatter( OutputStream *o, const char *preLine = ":", const char *postLine = "" ) :
            _dotting( true ),
            _reported( false ),
            _o(o),
            _preLine(preLine),
        ...
        void leaveTest( const TestDescription & t )
        {
            if ( !tracker().testFailed() ) 
            {
              char message[1256];
              sprintf_s( message, "Runtest %s %s %s Endtest\n",t.file(),t.suiteName(),t.testName());
              OutputDebugString(message);
              (*_o) << "Runtest " << t.file() << " " << t.suiteName() << " " << t.testName() << " Endtest\n";
                            _o->flush();
              _dotting = true;
            }
        }
    ...
    private:
        VerboseFormatter( const VerboseFormatter & );
        VerboseFormatter &operator=( const VerboseFormatter & );
        ...
        OutputStream &stop( const char *file, unsigned line )
        {
            ...
            (*_o) << "$_file_$ " << file << " $_line_$ " << line << " $_test_$ " << tracker().suite().suiteName() << "::" << tracker().test().testName() << ": $_error_$ ";
            return (*_o);
        }
...
#endif // __cxxtest__VerboseFormatter_h__
