...
namespace CxxTest 
{
                                              ...
                                              const char *argTestName )
    {
        initialize( argList, argSuite, argLine, argTestName );
				delete [] lowfile;
				lowfile = new char[strlen(_suite->file())+1];
				for ( unsigned int i = 0; i < strlen(_suite->file()); i++ )
				{
					lowfile[i] = tolower( _suite->file()[i] );
				}

				lowfile[strlen(_suite->file())] = 0;
    }
    ...
    const char *RealTestDescription::file() const { return lowfile; }
    ...
    bool StaticSuiteDescription::setUp() { _suite->setUpSuite(); return true; }
    bool StaticSuiteDescription::tearDown() { _suite->tearDownSuite(); return true; }
