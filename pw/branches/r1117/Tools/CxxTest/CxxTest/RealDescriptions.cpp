...
namespace CxxTest 
{
    ...
    bool RealTestDescription::setUp()
    {
        ...
        //HACK: our global fixtures lives outside CxxTest    
        if ( !_suite->setUpGlobalFixtures() ) {
				...
				bool ok = false;
        _TS_TRY {
            _TSM_ASSERT_THROWS_NOTHING( file(), line(), "Exception thrown from setUp()", ok = suite()->setUp() );
        }
        _TS_CATCH_ABORT( { return false; } );

				if ( !ok )
						doFailTest( file(), line(), "Error in suite setUp()" );
    ...
    bool RealTestDescription::tearDown()
    {
        if ( !suite() )
            return false;
				...
				bool ok = false;
        _TS_TRY {
            _TSM_ASSERT_THROWS_NOTHING( file(), line(), "Exception thrown from tearDown()", ok = suite()->tearDown() );
        }
        _TS_CATCH_ABORT( { return false; } );

				if ( !ok )
				{
						doFailTest( file(), line(), "Error in suite tearDown()" );
						return false;
				}
        
        //HACK: our global fixtures lives outside CxxTest    
        if ( !_suite->tearDownGlobalFixtures() ) {
    ...
    bool RealSuiteDescription::leaveOnly( const char *testName )
    {
		...
		//HACK: we implement descriptions in CxxRunner not in CxxTest library
